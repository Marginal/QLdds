...
OSStatus GeneratePreviewForURL(void *thisInterface, QLPreviewRequestRef preview, CFURLRef url, CFStringRef contentTypeUTI, CFDictionaryRef options)
{
...
#ifdef DEBUG
    NSLog(@"Preview %@ with options %@", [(NSURL*)url path], options);
#endif
    ...
    NSNumber *previewMode = (NSNumber*)[(NSDictionary*)options objectForKey:@"QLPreviewMode"];
    CGImageRef image;
    ...
    CGContextRef context = QLPreviewRequestCreateContext(preview, CGSizeMake(CGImageGetWidth(image), CGImageGetHeight(image)),
                                                         true, (CFDictionaryRef) properties);
